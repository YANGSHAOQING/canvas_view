...
//
	...
	bool CRenderEngine::UnZipFile2Buffer(const WCHAR* zipfile, vector<pair<wstring, string>> & vec)
	{
		HZIP hz = OpenZip((TCHAR*)zipfile, NULL);
		...
		for (int i = 0;; i++)
		{
			...
			int res = UnzipItem(hz, i, buffer, ze.unc_size);
			if (res != 0x00000000 && res != 0x00000600)
			{
				delete[] buffer;
				continue;
			}
			wstring name = ze.name;
			string ctx(buffer, ze.unc_size);
			pair<wstring, string> tmp;
			tmp.first = name;
			tmp.second = ctx;
			vec.push_back(tmp);
			delete [] buffer;
	...
	bool CRenderEngine::UnZipMem2Buffer(char* buf, int len, vector<pair<wstring, string>> & vec)
	{
		HZIP hz = OpenZip((void*)buf,NULL);
		DWORD dw = GetLastError();
		if (hz == NULL)
		{
			return false;

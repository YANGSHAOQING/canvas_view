...
public:
	...
	//
	static bool UnZipFile2Buffer(const WCHAR* zipfile, std::vector<std::pair<std::wstring, std::string>> & vec);
	static bool UnZipMem2Buffer(char* buf, int len, vector<pair<wstring, string>> & vec);
};

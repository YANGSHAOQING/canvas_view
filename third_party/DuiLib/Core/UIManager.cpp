...
//#endif
    ...
    if (!::IsWindow(m_hWndPaint)) return false;
   ...
   case WM_PAINT:
	   {
		   ...
		   if(m_bUpdateNeeded)
		   {
			   ...
			   if(!::IsRectEmpty(&rcClient))
			   {
				   if(m_pRoot->IsUpdateNeeded())
				   {
					   ...
					   m_hbmpOffscreen = NULL;
					   m_pBmpOffscreenBits = NULL;
                       SendNotify(m_pRoot, _T("windowrelayout"), 0, 0, false);
				   }
				   else
				   {
		   ...
		   if(m_bAlphaBackground)
		   {
               ...
               {
                   printf("CRenderEngine::ClearAlphaPixel failed");
               }
               ...
               {
                   printf("CRenderEngine::RestoreAlphaColor failed");
               }

			   if (m_pCall)
			   {
				   m_pCall->PM_PaintCallBack(m_hDcOffscreen, rcClient.right - rcClient.left,
		   ...
		   else
		   {
			   ...
			   if(m_bOffscreenPaint)
			   {
				   ...
				   //
#if 0
				   BitBlt(ps.hdc, rtWindow.left, rtWindow.top, rtWindow.right - rtWindow.left, rtWindow.bottom - rtWindow.top, m_hDcOffscreen, 0, 0, SRCCOPY);
#else
				   if (m_pCall && m_bNeedFlash)
				   {
					   RECT rtWindow;
					   GetClientRect(m_hWndPaint, &rtWindow);
					   
					   m_pCall->PM_FlashProc(ps.hdc, m_hDcOffscreen, m_hbmpOffscreen, rtWindow.right - rtWindow.left, rtWindow.bottom - rtWindow.top);
					   //BitBlt(ps.hdc, rtWindow.left, rtWindow.top, rtWindow.right - rtWindow.left, rtWindow.bottom - rtWindow.top, m_hDcOffscreen, 0, 0, SRCCOPY);
					   m_bNeedFlash = false;
				   }
				   else
    ...
    case WM_KEYDOWN:
        {
            ...
            event.dwTimestamp = ::GetTickCount();
			//按钮不转发按键时间
			if (_tcsicmp(m_pFocus->GetClass(), _T("ButtonUI")) != 0)
			{
				m_pFocus->Event(event);
			}
...
void CPaintManagerUI::SetCallBack(CPaintManagerUI_PaintCallBack* pCall)
{
	m_pCall = pCall;
}
} // namespace DuiLib

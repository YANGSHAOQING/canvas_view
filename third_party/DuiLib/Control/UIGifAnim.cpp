...
namespace DuiLib
{
	...
	void CGifAnimUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("bkimage")) == 0 ) SetBkImage(pstrValue);
			...
			SetLoop(_tcscmp(pstrValue, _T("true")) == 0);
		}
		else
	...
	void CGifAnimUI::OnTimer( UINT_PTR idEvent )
	{
		...
		if ( lPause == 0 ) lPause = 100;
           ...
           if (m_nFramePosition == m_nFrameCount) {
			if (m_bIsLoop == false) {
				m_pManager->SetTimer(this, EVENT_TIEM_ID, 1000000);
			}*/
		if (m_bIsLoop) {
			m_pManager->SetTimer(this, EVENT_TIEM_ID, lPause);
		}
		else {
			if (m_nFramePosition != (m_nFrameCount -1)) {
				m_pManager->SetTimer(this, EVENT_TIEM_ID, lPause);
			}
			else {
				m_pManager->SetTimer(this, EVENT_TIEM_ID, 10000000);
			}
	...
	void CGifAnimUI::SetLoop(bool bIsLoop)
	{
		m_bIsLoop = bIsLoop;
	}

...
namespace DuiLib
{
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_dwEditTextColor(0x00000000), m_iWindowStyls(0),m_dwTipValueColor(0xFFBAC0C5), m_dwFocusTextColor(0)
	{
	...
	void CEditUI::SetFocusTextColor(LPCTSTR pStrColor)
	{
		if (*pStrColor == _T('#')) pStrColor = ::CharNext(pStrColor);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pStrColor, &pstr, 16);

		m_dwFocusTextColor = clrColor;
	}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("focustextcolor")) == 0 ) SetFocusTextColor(pstrValue);
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		if (IsFocused() && m_dwFocusTextColor != 0)
		{
			DWORD mCurTextColor = m_dwFocusTextColor;
		}
		else
		{
			DWORD mCurTextColor = m_dwTextColor;

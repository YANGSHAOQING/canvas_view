...
LRESULT CALLBACK CShadowUI::ParentProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOVE:
		if(pThis->m_Status & SS_VISABLE)
		{
			...
			{
                SetWindowPos(pThis->m_hWnd, GetParent(pThis->m_hWnd),
...
void CShadowUI::MakeShadow(UINT32 *pShadBits, HWND hParent, RECT *rcParent)
{
	...
	RECT rect;
	GetRgnBox(hParentRgn, &rect);

	//父窗口尺寸已经发生变化了 不做处理了避免崩溃  
	if (rect.right != szParent.cx || rect.bottom != szParent.cy)
		return;
...
void CShadowUI::HideShadow(bool bShow)
{
	if (m_bIsShowShadow == bShow) return;

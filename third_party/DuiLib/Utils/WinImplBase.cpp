...
//	return S_FALSE;
	...
	{
		CControlUI* pbtnMax     = static_cast<CControlUI*>(m_PaintManager.FindControl(_T("btnMax")));       // 最大化按钮
		CControlUI* pbtnRestore = static_cast<CControlUI*>(m_PaintManager.FindControl(_T("btnRestore")));   // 还原按钮
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	case UILIB_ZIPRESOURCE:
		{
			HRSRC hResource = ::FindResource(m_PaintManager.GetResourceDll(), GetResourceID(), _T("ZIP"));
			if( hResource == NULL )
				return 0L;
			DWORD dwSize = 0;
	...
	if (pRoot==NULL)
	{
		MessageBox(NULL,_T("加载资源文件失败"),_T("UI"),MB_OK|MB_ICONERROR);
		ExitProcess(1);
		return 0;
	}

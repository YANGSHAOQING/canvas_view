...
namespace DuiLib
{
	CHorizontalLayoutUI::CHorizontalLayoutUI() : m_iSepWidth(0), m_uButtonState(0), m_bImmMode(false)
	{
		...
		m_CursorSize = ::LoadCursor(NULL, MAKEINTRESOURCE(IDC_SIZEWE));
	}
		...
		if( m_iSepWidth != 0 ) {
			...
			if( event.Type == UIEVENT_MOUSEMOVE )
			{
				if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
					...
					if (m_pManager) m_pManager->SendNotify(this, DUI_MSGTYPE_VALUECHANGED_MOVE, 0, 0, true);
			
					if (m_CursorSize != ::GetCursor())
					{
